...
if [ ! -f "$HOST_WEB_DIR/index.html" ]; then
  ...
  echo "Usa: HOST_WEB_DIR=/ruta/a/frontend bash instalar_silabo_pretty_v7_live.sh"
  exit 1
fi

echo "=== Backup index.html host ==="
cp "$HOST_WEB_DIR/index.html" "$HOST_WEB_DIR/index.html.bak.silabo_pretty_v7_$(date +%Y%m%d%H%M%S)"

cat > "$HOST_WEB_DIR/jomelai-syllabus-pretty-v7-live.js" <<'JS'
...
echo "=== Limpiar scripts anteriores de sílabo en host sin sed -i ==="
awk '
  index($0, "jomelai-syllabus-full-structure.js") == 0 &&
  index($0, "jomelai-syllabus-pretty-v5.js") == 0 &&
  index($0, "jomelai-syllabus-pretty-v6.js") == 0 &&
  index($0, "jomelai-syllabus-pretty-v7-live.js") == 0
' "$HOST_WEB_DIR/index.html" > "$HOST_WEB_DIR/index.html.tmp"

mv "$HOST_WEB_DIR/index.html.tmp" "$HOST_WEB_DIR/index.html"

echo "=== Inyectar v7 al final del host ==="
awk -v stamp="$STAMP" '
  index($0, "</body>") {
    print "    <script src=\"/jomelai-syllabus-pretty-v7-live.js?v=" stamp "\"></script>"
    print $0
    next
  }
  { print $0 }
...
echo "=== Copiar al contenedor si está activo ==="

if docker ps --format '{{.Names}}' | grep -q "^${FRONTEND_CONTAINER}$"; then
  docker cp "$HOST_WEB_DIR/index.html" "$FRONTEND_CONTAINER:/tmp/index.html.v7"
  docker cp "$HOST_WEB_DIR/jomelai-syllabus-pretty-v7-live.js" "$FRONTEND_CONTAINER:/tmp/jomelai-syllabus-pretty-v7-live.js"
...
cp index.html "index.html.bak.silabo_pretty_v7.$(date +%Y%m%d%H%M%S)"
cp /tmp/index.html.v7 "$DIR/index.html"
cp /tmp/jomelai-syllabus-pretty-v7-live.js "$DIR/jomelai-syllabus-pretty-v7-live.js"

nginx -t
nginx -s reload 2>/dev/null || true

echo "OK v7 instalado en contenedor"
grep -n "jomelai-syllabus-pretty-v7-live" index.html || true
IN
...
echo "Ahora debe verse el sílabo armándose en vivo dentro de la hoja bonita."
