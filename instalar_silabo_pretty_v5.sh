...
echo "=== Detectando index.html del host ==="
...
if [ -z "$HOST_WEB_DIR" ] || [ ! -f "$HOST_WEB_DIR/index.html" ]; then
  echo "ERROR: no encontre index.html del host."
  echo "Usa:"
  echo "HOST_WEB_DIR=/ruta/a/tu/frontend bash instalar_silabo_pretty_v5.sh"
  exit 1
fi

echo "Host web dir: $HOST_WEB_DIR"

cp "$HOST_WEB_DIR/index.html" "$HOST_WEB_DIR/index.html.bak.silabo_pretty_v5.$(date +%Y%m%d%H%M%S)"

cat > "$HOST_WEB_DIR/jomelai-syllabus-pretty-v5.js" <<'JS'
...
echo "=== Inyectando en index.html del host ==="

# Quitar scripts de sílabo anteriores que reemplazaban el resultado o mostraban JSON crudo.
sed -i '/jomelai-syllabus-full-structure.js/d' "$HOST_WEB_DIR/index.html"
sed -i '/jomelai-syllabus-pretty-v5.js/d' "$HOST_WEB_DIR/index.html"

STAMP="$(date +%s)"

if grep -q '<script src="/tech-routes.js"></script>' "$HOST_WEB_DIR/index.html"; then
  sed -i "s#<script src=\"/tech-routes.js\"></script>#<script src=\"/tech-routes.js\"></script>\n    <script src=\"/jomelai-syllabus-pretty-v5.js?v=$STAMP\"></script>#" "$HOST_WEB_DIR/index.html"
elif grep -q '</body>' "$HOST_WEB_DIR/index.html"; then
  sed -i "s#</body>#    <script src=\"/jomelai-syllabus-pretty-v5.js?v=$STAMP\"></script>\n  </body>#" "$HOST_WEB_DIR/index.html"
else
  echo "<script src=\"/jomelai-syllabus-pretty-v5.js?v=$STAMP\"></script>" >> "$HOST_WEB_DIR/index.html"
fi

echo "=== Copiando al contenedor si esta activo ==="
...
cp index.html "index.html.bak.silabo_pretty_v5.$(date +%Y%m%d%H%M%S)"
cp /tmp/jomelai-syllabus-pretty-v5.js "$DIR/jomelai-syllabus-pretty-v5.js"

sed -i '/jomelai-syllabus-full-structure.js/d' index.html
sed -i '/jomelai-syllabus-pretty-v5.js/d' index.html

STAMP="$(date +%s)"

if grep -q '<script src="/tech-routes.js"></script>' index.html; then
  sed -i "s#<script src=\"/tech-routes.js\"></script>#<script src=\"/tech-routes.js\"></script>\n    <script src=\"/jomelai-syllabus-pretty-v5.js?v=$STAMP\"></script>#" index.html
elif grep -q '</body>' index.html; then
  sed -i "s#</body>#    <script src=\"/jomelai-syllabus-pretty-v5.js?v=$STAMP\"></script>\n  </body>#" index.html
else
  echo "<script src=\"/jomelai-syllabus-pretty-v5.js?v=$STAMP\"></script>" >> index.html
fi

nginx -t
nginx -s reload 2>/dev/null || true

echo "OK instalado en contenedor"
...
echo "El sílabo debe verse estructurado, editable y con botón Continuar generación."
