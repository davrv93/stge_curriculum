...
(function(){
  ...
  window.setupTestChartCompat = async function(){
    ...
    try { const r = await compat(['/api/charts/natural'], {method:'POST', body:JSON.stringify({ model:'llama3.2:1b', table:val('setupTable','silabos'), question:val('setupChartQuestion'), limit:300 })}); htmlOut('setupChartOutput', renderChartCompat(r)); }
  ...
  window.naturalSqlCompat = async function(){ htmlOut('natOutput','Consultando…'); try{ const p={model:val('natModel','llama3.2:1b'),table:val('natTable','silabos'),question:val('natQuestion'),limit:num('natLimit',100)}; const r=await compat(['/api/engine/natural-sql','/api/duckdb/natural-sql'],{method:'POST',body:JSON.stringify(p)}); htmlOut('natOutput', `<h4>SQL generado</h4><pre>${esc2(r.sql || '')}</pre>` + renderRows(r.rows || [])); }catch(e){ htmlOut('natOutput',`<pre>${esc2(e.message)}</pre>`); } };
  ...
  window.ragAnswerCompat = async function(){ htmlOut('ragAnswerOutput','Consultando…'); try{ const p={model:val('ragAnswerModel','llama3.2:1b'),collection:val('ragAnswerCollection','silabos'),question:val('ragQuestion'),n_results:num('ragAnswerN',5)}; const r=await compat(['/api/engine/rag-answer','/api/chat-lateral/ask'],{method:'POST',body:JSON.stringify(p)}); htmlOut('ragAnswerOutput', `<pre>${esc2(r.answer || r.response || r.message || JSON.stringify(r,null,2))}</pre>`); }catch(e){ htmlOut('ragAnswerOutput',`<pre>${esc2(e.message)}</pre>`); } };
  ...
  window.naturalChartCompat = async function(){ htmlOut('naturalChartOutput','Generando…'); try{ const p={model:val('chartModel','llama3.2:1b'),table:val('chartTable','silabos'),question:val('chartQuestion'),limit:300}; const r=await compat(['/api/charts/natural'],{method:'POST',body:JSON.stringify(p)}); htmlOut('naturalChartOutput', renderChartCompat(r)); }catch(e){ htmlOut('naturalChartOutput',`<pre>${esc2(e.message)}</pre>`); } };
