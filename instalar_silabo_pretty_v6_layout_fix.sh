...
if [ ! -f "$HOST_WEB_DIR/index.html" ]; then
  ...
  echo "Usa: HOST_WEB_DIR=/ruta/a/frontend bash instalar_silabo_pretty_v6_layout_fix.sh"
  exit 1
fi

echo "=== Backup index.html host ==="
cp "$HOST_WEB_DIR/index.html" "$HOST_WEB_DIR/index.html.bak.silabo_pretty_v6_$(date +%Y%m%d%H%M%S)"

cat > "$HOST_WEB_DIR/jomelai-syllabus-pretty-v6.js" <<'JS'
...
echo "=== Inyectar v6 al final del host ==="
awk -v stamp="$STAMP" '
  index($0, "</body>") {
    print "    <script src=\"/jomelai-syllabus-pretty-v6.js?v=" stamp "\"></script>"
    print $0
    next
  }
  { print $0 }
...
if docker ps --format '{{.Names}}' | grep -q "^${FRONTEND_CONTAINER}$"; then
  docker cp "$HOST_WEB_DIR/index.html" "$FRONTEND_CONTAINER:/tmp/index.html.v6"
  docker cp "$HOST_WEB_DIR/jomelai-syllabus-pretty-v6.js" "$FRONTEND_CONTAINER:/tmp/jomelai-syllabus-pretty-v6.js"
...
cp index.html "index.html.bak.silabo_pretty_v6.$(date +%Y%m%d%H%M%S)"
cp /tmp/index.html.v6 "$DIR/index.html"
cp /tmp/jomelai-syllabus-pretty-v6.js "$DIR/jomelai-syllabus-pretty-v6.js"

nginx -t
nginx -s reload 2>/dev/null || true

echo "OK v6 instalado en contenedor"
grep -n "jomelai-syllabus-pretty-v6" index.html || true
IN
...
echo "El sílabo debe quedar en una sola hoja centrada, sin panel lateral, editable y con botón Continuar generación."
