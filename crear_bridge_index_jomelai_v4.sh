...
HOST_WEB_DIR="${HOST_WEB_DIR:-}"

echo "=== Detectar carpeta web del host ==="

if [ -z "$HOST_WEB_DIR" ]; then
  for d in . ./frontend ./frontend/public ./public ./dist ./html ./src; do
...
if [ -z "$HOST_WEB_DIR" ] || [ ! -f "$HOST_WEB_DIR/index.html" ]; then
  echo "ERROR: no encontre index.html en el host."
  echo "Ejecuta asi:"
  echo "HOST_WEB_DIR=/ruta/a/tu/frontend bash crear_bridge_index_jomelai_v4.sh"
  exit 1
fi

echo "Host web dir: $HOST_WEB_DIR"

cp "$HOST_WEB_DIR/index.html" "$HOST_WEB_DIR/index.html.bak.bridge_v4.$(date +%Y%m%d%H%M%S)"

cat > "$HOST_WEB_DIR/jomelai-index-bridge-v4.js" <<'JS'
...
echo "=== Inyectar script en index.html del host ==="

sed -i '/jomelai-index-bridge-v4.js/d' "$HOST_WEB_DIR/index.html"

STAMP="$(date +%s)"

if grep -q '<script src="/tech-routes.js"></script>' "$HOST_WEB_DIR/index.html"; then
  sed -i "s#<script src=\"/tech-routes.js\"></script>#<script src=\"/tech-routes.js\"></script>\n    <script src=\"/jomelai-index-bridge-v4.js?v=$STAMP\"></script>#" "$HOST_WEB_DIR/index.html"
elif grep -q '</body>' "$HOST_WEB_DIR/index.html"; then
  sed -i "s#</body>#    <script src=\"/jomelai-index-bridge-v4.js?v=$STAMP\"></script>\n  </body>#" "$HOST_WEB_DIR/index.html"
else
  echo "<script src=\"/jomelai-index-bridge-v4.js?v=$STAMP\"></script>" >> "$HOST_WEB_DIR/index.html"
fi

echo "=== Copiar tambien al contenedor si existe ==="

if docker ps --format '{{.Names}}' | grep -q "^${FRONTEND_CONTAINER}$"; then
  docker cp "$HOST_WEB_DIR/jomelai-index-bridge-v4.js" "$FRONTEND_CONTAINER:/tmp/jomelai-index-bridge-v4.js"
  docker cp "$HOST_WEB_DIR/index.html" "$FRONTEND_CONTAINER:/tmp/index.html.bridge_v4"
...
cp index.html "index.html.bak.bridge_v4.$(date +%Y%m%d%H%M%S)"
cp /tmp/jomelai-index-bridge-v4.js "$DIR/jomelai-index-bridge-v4.js"

sed -i '/jomelai-index-bridge-v4.js/d' index.html

STAMP="$(date +%s)"

if grep -q '<script src="/tech-routes.js"></script>' index.html; then
  sed -i "s#<script src=\"/tech-routes.js\"></script>#<script src=\"/tech-routes.js\"></script>\n    <script src=\"/jomelai-index-bridge-v4.js?v=$STAMP\"></script>#" index.html
elif grep -q '</body>' index.html; then
  sed -i "s#</body>#    <script src=\"/jomelai-index-bridge-v4.js?v=$STAMP\"></script>\n  </body>#" index.html
else
  echo "<script src=\"/jomelai-index-bridge-v4.js?v=$STAMP\"></script>" >> index.html
fi

nginx -t
nginx -s reload 2>/dev/null || true

echo "OK instalado en contenedor: $DIR"
grep -n "jomelai-index-bridge-v4" index.html || true
IN

  docker compose restart frontend 2>/dev/null || docker restart "$FRONTEND_CONTAINER"
else
  echo "AVISO: contenedor $FRONTEND_CONTAINER no esta activo; solo se actualizo el host."
fi

echo ""
echo "Listo."
echo "Ahora recarga con Ctrl+F5."
echo "Verifica que el HTML cargue /jomelai-index-bridge-v4.js al final."
